    ...
    return {
        ...
        hexToRGBArray: function (hexColor) {
            var normalizedColor = this.normalizeHexColor(hexColor),
                hexToDec = function (offset) {
                    return parseInt(normalizedColor.slice(offset, offset + 2), 16);
                };
            return [
                hexToDec(0),
                hexToDec(2),
        ...
        normalizeHexColor: function (hexColor) {
            return hexColor.replace("#", "").replace(/^(.)(.)(.)$/, "$1$1$2$2$3$3");
        },
