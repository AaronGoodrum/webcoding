...
var angularObj = {
    ...
    initAngular: function () {
            ...
            return {
                ...
                link: function (scope, element, attrs) {
                    ...
                    scope.hideModal = function () {
                        scope.show = false;
                        document.getElementById("importKMLZonesId").style.overflow = "";
                        document.getElementById("importKMLZonesId").style.height = "auto";
                    };
                },
            ...
            $scope.toggleModal = function () {
                var showZoneTypes = function (zoneTypes) {
                    ...
                    kml.options.zoneTypes.every(function (type, index) {
                        if (type === kml.defaultZoneType &&
                            document.getElementById("typesSelect").selectedIndex === -1) {
                            document.getElementById("typesSelect").selectedIndex = index;
                            return false;
                        }
                        return true;
                    });
                ...
                if (kml.utils.inputTypeSupport("range", "a")) {
                    document.querySelector(".vanillaSlider").value = $scope.transparencySliderValue;
                }
                angular.element(document.querySelector("input.vanillaSlider")).on("input", function (event) {
                    var val = event.target.value;
                    $scope.transparencySliderValue = Number(val);
                    kml.colorPicker.getTransparencyControl().set({a: val});
                    document.getElementById("colorPickerField").style.opacity = ((100 - val) / 100);
                    $scope.$apply();
                });
                ...
                if ($scope.modalShown === true) {
                    document.getElementById("importKMLZonesId").style.overflow = "hidden";
                    document.getElementById("importKMLZonesId").style.height = "100%";
                }
            };
            ...
            $scope.selectType = function (event) {
                ...
                setTimeout(function () {
                    ...
                    for (i = 0; i < kml.prevSelectedTypes.length; i++) {
                        document.getElementById("typesSelect").options[kml.prevSelectedTypes[i]].selected = true;
                    }
                    //unselect current item if already selected
                    if (unselect !== null) {
                        document.getElementById("typesSelect").options[unselect].selected = false;
                    }
                }, 0);
                kml.selectedTypes = document.getElementById("typesSelect").selectedOptions ||
        ...
        angularObj.app.controller("parsedDataController", ["$scope", "clearService", function ($scope, clearService) {
            if (kml.isFormDataSupported) {
                $scope.uploaderTitle = "Drag and drop your files here or click and choose them";
            } else {
                $scope.uploaderTitle = "Click here to choose kml file";
            }
